        ...
        if (file.is_open()) {
            while (getline(file, line)) {
                ...
                if (findInList(AllRobotType, line, RobotType)){
                    ...
                    if (line.find(" ")){
                        // Check their name
                        robot_symbol = line.substr(0,1);
                        robot_name = line.substr(0,line.find(" "));
                        // Add the first letter of their name into a vector named 'RobotSymbols'
                        RobotSymbols.push_back(robot_symbol);
            ...
            outputFile << " )\\ ) ( /(   (  ( /( (  (      (     )\\ ) )\\ )\n";
            outputFile <<"(()/( )\\())( )\\ )\\()))\\))(   ' )\\   (()/((()/(\n";
            outputFile << " /(_)|(_)\\ )((_|(_)\\((_)()\\ |(((_)(  /(_))/(_))\n";
            outputFile << "(_))   ((_|(_)_  ((_)(())\\_)()\\ _ )\\(_)) (_))\n";
            outputFile << "| _ \\ / _ \\| _ )/ _ \\ \\((_)/ (_)_\\(_) _ \\/ __|\n";
            outputFile << "|   /| (_) | _ \\ (_) \\ \\/\\/ / / _ \\ |   /\\__ \\\n";
            outputFile << "|_|_\\ \\___/|___/\\___/ \\_/\\_/ /_/ \\_\\|_|_\\|___/\n";
            ...
            for(int y = 1; y <= row; ++y){
                ...
                outputFile << "*\n";
                    for (int i = 1; i <= column; ++i){
                        ...
                        number_check = findInListIndex(RobotPositionX, RobotPositionY, i, y);
                        if(number_check != -1){
                            cout << row_box + " " << RobotSymbols[number_check] << " ";
                            outputFile << row_box + " " << RobotSymbols[number_check] << " ";
                        }
                        else{
                            cout << row_box + "   ";
                            outputFile << row_box + "   ";
            ...
            for (int i = 0; i < robot_num; ++i){
                if (RobotType[i] == "HideBot"){
                    GenericRobots.push_back(new HideBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "JumpBot"){
                    GenericRobots.push_back(new JumpBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "SprintBot"){
                    GenericRobots.push_back(new SprintBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "LongShotBot"){
                    GenericRobots.push_back(new LongShotBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "SemiAutoBot"){
                    GenericRobots.push_back(new SemiAutoBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "ThirtyShotBot") {
                    // Set to 30 for ThirtyShotBot
                    GenericRobots.push_back(new ThirtyShotBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 30, 3));
                    RobotShells.push_back(30);
                }
                else if (RobotType[i] == "PreciseShotBot"){
                    GenericRobots.push_back(new PreciseShotBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "ScoutBot"){
                    GenericRobots.push_back(new ScoutBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "TrackBot"){
                    GenericRobots.push_back(new TrackBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "EagleEyeBot"){
                    GenericRobots.push_back(new EagleEyeBot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                    RobotShells.push_back(10);
                }
                else if (RobotType[i] == "GenericRobot"){
                    GenericRobots.push_back(new GenericRobot(RobotType[i], RobotNames[i], RobotPositionX[i], RobotPositionY[i], 3, 10, 3));
                ...
                RobotRespawn.push_back(3);
                PastUpgrades.push_back({"Null","Null","Null"});
            }
            ...
            while(counter <= steps){
                ...
                for (int i = 0; i < robot_num; ++i) {
                    cout << "[ROUND " << i + 1 << "]\n";
                    outputFile << "[ROUND " << i + 1 << "]\n";
                    // Respawn the robot
                    if(GenericRobots[i]->ShowX() <= -5 && respawn == true && RobotRespawn[i] > 0){
                        int new_x, new_y;
                        new_x = (rand() % row) + 1;
                        new_y = (rand() % column) + 1;
                        ...
                        int oldRespawns = RobotRespawn[i];

                        // Delete old robot object
                        delete GenericRobots[i];
                        GenericRobots[i] = nullptr;

                        GenericRobots[i] = new GenericRobot("GenericRobot",oldName, new_x, new_y, 3, 10, oldRespawns);
                    ...
                    else if(GenericRobots[i]->ShowX() <= -5 && respawn == false && RobotRespawn[i] > 0){
                        GenericRobots[i]->CannotRespawnQuote();
                    }
                    ...
                    if(how_many_lives == 0){
                        ...
                        RobotAlive[which_robot] = false;
                        RobotRespawn[which_robot] -= 1;
                    }
                    if(RobotRespawn[i] <= 0){
                        GenericRobots[i]->KilledOffQuote();
                    }
                    cout << "\n";
                    outputFile << "\n";
                    ...
                    if (RobotUpgrade[i]==true && UpgradeCount[i] < 3) {
                        ...
                        while(randomizer == true){
                            abilityType = rand() % 3;  // 0 = moving, 1 = shooting, 2 = seeing

                            if (abilityType == 0 && !findInListStr(PastUpgrades[i], "Moving")) {
                                chosenType = MovingRobotType[rand() % MovingRobotType.size()];
                                PastUpgrades[i][0] = "Moving";
                                randomizer = false;
                            }
                            else if (abilityType == 1 && !findInListStr(PastUpgrades[i], "Shooting")) {
                                chosenType = ShootingRobotType[rand() % ShootingRobotType.size()];
                                PastUpgrades[i][1] = "Shooting";
                                randomizer = false;
                            }
                            else if (abilityType == 2 && !findInListStr(PastUpgrades[i], "Seeing")) {
                                chosenType = SeeingRobotType[rand() % SeeingRobotType.size()];
                                PastUpgrades[i][2] = "Seeing";
                                randomizer = false;
                            }
                        }
                        ...
                        if (!chosenType.empty()) {
                            ...
                            outputFile << RobotType[i] << " " << RobotNames[i] << " upgrades and transforms into a " << chosenType << "!\n";
                            ...
                            if (chosenType == "HideBot") {
                                GenericRobots[i] = new HideBot("HideBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "JumpBot") {
                                GenericRobots[i] = new JumpBot("JumpBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "SprintBot") {
                                GenericRobots[i] = new SprintBot("SprintBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "LongShotBot") {
                                GenericRobots[i] = new LongShotBot("LongShotBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "SemiAutoBot") {
                                GenericRobots[i] = new SemiAutoBot("SemiAutoBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "ThirtyShotBot") {
                                GenericRobots[i] = new ThirtyShotBot("ThirtyShotBot",oldName, oldX, oldY, oldLives, 30, oldRespawns);
                                RobotShells[i] = 30;
                            }
                            else if (chosenType == "PreciseShotBot") {
                                GenericRobots[i] = new PreciseShotBot("PreciseShotBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "ScoutBot") {
                                GenericRobots[i] = new ScoutBot("ScoutBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "TrackBot") {
                                GenericRobots[i] = new TrackBot("TrackBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else if (chosenType == "EagleEyeBot") {
                                GenericRobots[i] = new EagleEyeBot("EagleEyeBot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
                            else {
                                // fallback to generic robot
                                GenericRobots[i] = new GenericRobot("GenericRobot",oldName, oldX, oldY, oldLives, oldShells, oldRespawns);
                            }
