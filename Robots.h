...
public:
    ...
    void think(int x, int y, int index) override {
        if (RobotShells[index] == 0 || !RobotAlive[index]) {
            GenericRobot::think(x, y, index); // fallback for death or self-destruction
            return;
        }
    ...
    void think(int x, int y, int index) override {
        ...
        for (int tx = 1; tx <= column; ++tx) {
            for (int ty = 1; ty <= row; ++ty) {
                ...
                if (manhattan <= 3 && findInListInt(RobotPositionX, RobotPositionY, tx, ty)) {
                    fire(tx, ty, index);
                    return;
                }
            }
    ...
    void think(int x, int y, int index) override {
        ...
        if (trackers_left > 0) {
            for (int i = 0; i < RobotPositionX.size(); ++i) {
                ...
                int dx = abs(self_x - RobotPositionX[i]);
                int dy = abs(self_y - RobotPositionY[i]);

                if (dx <= 3 && dy <= 3) {
                    tracked_indices.push_back(i);
                    trackers_left--;
                    cout << "TrackBot " << RobotNames[index] << " planted a tracker on " << RobotNames[i]
        ...
        for (int tracked : tracked_indices) {
            ...
            cout << "TrackBot " << RobotNames[index] << " is tracking " << RobotNames[tracked]
            ...
            if (!findInListInt(RobotPositionX, RobotPositionY, move_x, move_y)) {
                ...
                cout << "TrackBot " << RobotNames[index] << " tried to move but the position was occupied.\n";
            }

            // Try to shoot the tracked robot
            fire(target_x, target_y, index);
            return;
        }

        // If nothing to track or fire, just look around
        cout << "TrackBot " << RobotNames[index] << " finds no tracked target.\n";
        look(self_x, self_y, index);
    }
};
